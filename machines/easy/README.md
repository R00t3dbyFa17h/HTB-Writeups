...
**Target:** _Bashed (Hack The Box)_ **OS:** _Linux_ **Difficulty:** _Easy_ **Attack Vectors:** _Exposed Web Shell (phpbash) -> Sudo Misconfiguration -> Insecure Automation._
...
Root compromise was achieved by enumerating local users and identifying a privilege escalation vector involving the **`scriptmanager`** user. By manipulating a Python script executed automatically by a system process _**(cron/sudo)**_, we injected a malicious payload to elevate privileges from a restricted user to **Root**.
...
* We began the assessment with a comprehensive Nmap scan to identify open ports and services on **`10.10.10.68`**.
...
* **Operating System:** The TCP fingerprint confirms the target is running Linux (Kernel 3.x — 4.x), likely Ubuntu based on the Apache version string.
* **Web Title:** The site title “Arrexel’s Development Site” is a significant clue. Development sites often contain unfinished code, debug tools, or backup files that should not be public.
* **Attack Vector:** With no other services (like SSH or SMB) exposed, our entire attack surface is the web application. We must focus on directory enumeration and inspecting the site for developer oversight.

**1.1.3 Key Findings**

* **Port 80:** Apache httpd 2.4.18.
* **OS:** Ubuntu Linux.
* **Context:** A “Development Site” implies potential security lapses (forgotten tools, weak permissions).
...
* I attempted to map the directories using **Gobuster**, but the target server was unstable and repeatedly timed out.
* Instead of waiting on a failing tool, I manually verified common directories based on the “Development Site” context. I successfully identified the **`/dev`** directory using **`curl`**.
...
1. **Payload Preparation:** I configured the PentestMonkey `p`**`hp-reverse-shell.php`** with my attacker IP (**`10.10.14.32`**) and port (**`4444`**).
2. **Delivery:** I hosted the file on my attack machine using a Python HTTP server and downloaded it to the target using **`wget`**.
...
* **Terminal Adjustment:** Used **`stty raw -echo; fg`** to pass keyboard shortcuts correctly.
...
* **Command:** **`ls -la /`**
* **Finding:** A directory named **`/scripts`**, owned by **`scriptmanager`**, located at the filesystem root.

**4.1.3 Analysis of** **`/scripts`:** Inside the directory, I found two files:

1. **`test.py`** (Owned by **`scriptmanager`**)
2. **`test.txt`** (Owned by **`root`**)
...
**4.1.5 Conclusion:** This indicates that a system cron job (scheduled task) is executing **`test.py`** as the **Root** user at regular intervals (likely every minute). Because I have write access to **`test.py`**, I can modify the code. When the cron job triggers again, it will execute _my_ code with Root privileges.
...
The initial foothold was achieved simply because a convenient administrative tool (**`phpbash`**) was left accessible to the public internet. From there, privilege escalation was trivialized by a misconfigured sudo permission and a maintenance script that was writable by a low-privileged user but executed by Root. The system was wide open not because of what it _was_, but because of what was _left behind_.
...
1. **Immediate Removal of Shadow IT:** The **`phpbash.php`** web shell must be deleted immediately. If remote administration is required, use secure, authenticated protocols like SSH with key-based authentication, not unauthenticated web scripts.
2. **Principle of Least Privilege (sudo):** Review the **`/etc/sudoers`** file. The **`www-data`** user should not have password-less access to switch to the **`scriptmanager`** account.
3. **Secure File Permissions:** The **`/scripts`** directory and its contents utilize unsafe permissions. A script executed by Root should **never** be writable by a standard user. Change the ownership of **`test.py`** to **`root:root`** and set permissions to **`700`** (read/write/execute for owner only).
4. **Cron Job Hygiene:** Audit all system cron jobs. Ensure that scripts triggered by system-wide crontabs are stored in protected directories (like **`/usr/local/sbin`**) rather than user-accessible folders.
...
> _**“One who is slack in his work is brother to one who destroys.” — Proverbs 18:9 (NIV)**_

**Application:** In the physical world, being “slack” or lazy might just result in a messy desk. In the digital world, being slack creates a direct path for destruction. The developer didn’t _intend_ to harm the server, but by failing to remove the temporary web shell and secure the python script, they became a “brother to one who destroys.”
...
By [Nicholas Mullenski](https://medium.com/@nicholasmullenski) on [December 31, 2025](https://medium.com/p/dc0ecda2c4ba).

[Canonical link](https://medium.com/@nicholasmullenski/%EF%B8%8F-%EF%B8%8F-hacking-bashed-the-deadly-cost-of-shadow-it-exposed-dc0ecda2c4ba)
